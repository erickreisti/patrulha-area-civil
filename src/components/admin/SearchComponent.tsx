...
export function SearchComponent({ className }: SearchComponentProps) {
  ...
  // Busca automática com debounce - corrigido para incluir performSearch na dependência
  const performSearch = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        // Buscar notícias - corrigido para incluir status no metadata
  ...
  // Foco no input de busca quando aberto
  useEffect(() => {
  ...
  // Busca automática com debounce - corrigida dependência
  useEffect(() => {
  ...
  }, [searchQuery, performSearch]); // Adicionado performSearch nas dependências
      ...
      <div className={`relative ${className}`} ref={searchAreaRef}>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-10 w-64 lg:w-80 border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-lg h-10 px-3 py-2 text-sm transition-all duration-200 bg-white"
            aria-label="Buscar no sistema"
            aria-haspopup="dialog"
            aria-controls="search-results-dialog"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        {/* Modal de resultados - removido aria-describedby para evitar o erro */}
        <Dialog open={isSearchOpen} onOpenChange={setIsSearchOpen}>
          <DialogContent
            className="max-w-2xl max-h-[80vh] overflow-hidden p-0 bg-white"
            // REMOVIDO: aria-describedby causava o erro quando DialogDescription não era renderizado
            // aria-describedby="search-results-description"
            id="search-results-dialog"
            onInteractOutside={(e) => {
              // Impede fechar ao clicar no próprio input
          ...
          >
            <DialogHeader className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-navy-700 to-navy-900">
              ...
              {/* Sempre renderiza DialogDescription para evitar erro de acessibilidade */}
              <DialogDescription className="text-blue-200 mt-1 sr-only">
                {searchQuery
                  ? `Resultados para "${searchQuery}"`
                  : "Painel de busca do sistema"}
              </DialogDescription>
              ...
              <AnimatePresence mode="wait">
                {isSearching ? (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-400 mt-2">
                      Busque por agentes, notícias, galeria, etc.
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>

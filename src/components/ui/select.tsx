...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      // Layout Base
      "flex h-11 w-full items-center justify-between rounded-xl border px-3 py-2 text-sm shadow-sm",
      // Cores (Mantém a borda cinza padrão)
      "border-slate-200 bg-white text-slate-700",
      // Placeholder
      "placeholder:text-slate-400 data-[placeholder]:text-slate-400",
      // REMOVIDO: Bordas coloridas e anéis de foco
      // ADICIONADO: focus:ring-0 para garantir que não apareça nada
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="h-4 w-4 text-slate-400 opacity-80 transition-transform duration-200 group-data-[state=open]:rotate-180" />
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Check className="h-4 w-4 text-pac-primary" />
      </SelectPrimitive.ItemIndicator>
    </span>
